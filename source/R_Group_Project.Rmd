...
tot_confirmed <- confirmed[, ncol(confirmed)]
tot_recoveries <- recovered[, ncol(recovered)]
tot_deaths <- deaths[, ncol(deaths)]

global_deaths <- sum(tot_deaths)
global_recoveries <- sum(tot_recoveries)
...
#Sort data frame by the second column

sort_data_frame <- function(df)
  
{
  
  column_2 <- colnames(df)[2]
  
  df[[column_2]] <- as.numeric(as.character(df[[column_2]]))
  
  df[order(-df[[column_2]]),]
...
total_confirmations_sorted <- sort_data_frame(total_confirmations)

total_recoveries_sorted <- sort_data_frame(total_recoveries)

total_deaths_sorted <- sort_data_frame(total_deaths)
