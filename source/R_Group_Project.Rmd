...
places2 <- paste0(recovered[ , 1], " ", recovered[ , 2])

for (i in 1:length(places))
{
  if (places[i] != places2[i])
  {
    print(places2[i])
  }
}
...
length(sums_confirmed) - length(sums_recoveries)

sums <- list(places, sums_confirmed, sums_recoveries, sums_deaths)
sums[["Afghanistan"]]

lowest_risk <- places[which.min(risk)]
hightest_risk <- places[which.max(risk)]
risk[which.min(risk)]
risk[which.max(risk)]
...
generate_total_df <- function(places, countries, total, sums, column_name)
{
  for (i in 1:length(places)){
    if (is.element(places[i], countries)){
      new <- match(places[i], countries)
      total[new] <- total[new] + sums[i] 
    }
    else{
      countries <- c(countries, places[i])
      total <- c(total, sums[i])
    }
  }
  total <- data.frame(countries, total)
  colnames(total) <- c("countries", column_name)
  total
}
...
places_c <- confirmed[, 2]
places_r <- recovered[, 2]
places_d <- deaths[, 2]
...
total_confirmations <- vector()
total_recoveries <- vector()
total_deaths <- vector()

# for every country in the original data sets
total_data <- generate_total_df(places_c, countries_c, total_confirmations, sums_confirmed, "total confirmations")
print(total_data)

total_data2 <- generate_total_df(places_r, countries_r, total_recoveries, sums_recoveries, "total recoveries")
print(total_data2)

total_data3 <- generate_total_df(places_d, countries_d, total_deaths, sums_deaths, "total deaths")
print(total_data3)
