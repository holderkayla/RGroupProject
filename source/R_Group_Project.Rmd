...
#colnames(confirmed)
unique_regions <- select(recovered, Province.State, Country.Region)
non_regions <- select(deaths, Province.State, Country.Region)
same <- merge(non_regions,unique_regions)

non_exist <- setdiff(non_regions, unique_regions)
...
col2 <- which(recovered_with_scores$risk_score < Inf)
recovered_with_scores[1]
#stuff <- recovered_with_scores[[which(recovered_with_scores$risk_score < Inf)]]

#max(stuff, na.rm = TRUE)
recovered_with_scores
#min(recovered_with_scores$risk_score, na.rm = TRUE )
...
filtered_locations_confirmed <- unique(confirmed[, 2])
filtered_locations_recovered <- unique(recovered[, 2])
filtered_locations_deaths <- unique(deaths[, 2])

get_totals <- function(data_set, place) {
  new_df <- data_set %>%
    filter(Country.Region == place) %>%
    select(Country.Region, length(data_set))
  
  return(sum(new_df[, 2]))
}

confirmed_totals <- lapply(filtered_locations, get_totals, data_set = confirmed)
confirmed_totals <- unlist(confirmed_totals)

recovered_totals <- lapply(filtered_locations, get_totals, data_set = confirmed)
recovered_totals <- unlist(confirmed_totals)

death_totals <- lapply(filtered_locations, get_totals, data_set = confirmed)
death_totals <- unlist(confirmed_totals)
...
#{
  ...
  #for (i in 1:length(places)){
    ...
    #}
  #}
  # filtered_places <- unique(tot_places)
  # func(dataset, place)
    # new_df <- chosen_dataset %>%
    #   filter(Country.Region == places) %>%
    #   select(country, last_column)
  # 
  # return(sum(new_df[,2]))
    #}
  #}
